...
void FaultTolerant::MakeSubsets(){
	for(int i=0; i<this->cff.M.size(); i++){
		for(int j=0; j<this->cff.M[i].size(); j++){
...
int main(int argc, char *argv[]){
	...
	for(int i=0;i<ft.aggs.size();i++){
		...
		if(aggok3){
			for(int j=0;j<ft.aggs[i].users.size();j++){
